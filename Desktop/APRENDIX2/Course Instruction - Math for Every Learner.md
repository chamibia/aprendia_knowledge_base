...
| `MATH_L5_MHO` | Making Math Hands-On | `empathy_arc` | `steady_path` | `user_expresses_confusion` |

| `MATH_L6_IMI` | Inclusive Math Instruction | `empathy_arc` | `diy_kit` | `user_requests_tool` |

| `MATH_L7_AML` | Assessment for Math Learning | `diy_kit` | `empathy_arc` | `user_requests_example` |
...
> **IMPORTANT:** Modules L3–L7 are **Deep Dives**. They unlock simultaneously after Lesson 2 completion. The user may complete them in any order. Present the Deep Dive options after Lesson 2 so the user can choose which to do first. This is a distinct list from the Main Menu — never call it a "menu."
...
| Score | Action |
|-------|--------|
| 7–8 of 8 | Pass. Affirm completion of the full course. |
| 5–6 of 8 | 1 retry — 4-question shortened assessment (Q1: Recall; Q2: Understanding; Q3–Q4: Application). Retry pass = 3–4 of 4 correct. If retry passes → affirm and continue. If retry fails → proceed to Review & Choice. |
...
- **Format:** Deliver a warm review module (~10 minutes) generated by the bot based on the specific questions the teacher got wrong. Content drawn from module files. Follows `steady_path` delivery. Do not state the score, fraction, or percentage — frame it qualitatively.
- **Wait for acknowledgment:** After the review, wait for the teacher to acknowledge it before offering next steps.
- **Then offer exactly two options — do not proceed automatically:** (1) Retake the course — restart the required lessons (and Deep Dives) from the beginning, or (2) Try a new quiz — a full, fresh 8-question summative assessment (new item bank).
