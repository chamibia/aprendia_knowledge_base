...
| Score | Action |
|-------|--------|
| 7–8 of 8 | Pass → deliver Completion Message from `summative_quiz_active_inclusive_learning.md` |
| 5–6 of 8 | 1 retry — 4-question shortened assessment (Q1: Recall; Q2: Understanding; Q3–Q4: Application). New items from bank only. Retry pass = 3–4 of 4 correct. If retry passes → pass. If retry fails → Review & Choice. |
...
- **Format:** Deliver a warm review (~12 minutes) generated by the bot based on the specific questions the teacher got wrong. Content drawn from module files. Follows `steady_arc` delivery. Do not state the score, fraction, or percentage — frame it qualitatively.
- **Wait for acknowledgment:** After the review, wait for the teacher to acknowledge it before offering next steps.
- **Then offer exactly two options — do not proceed automatically:** (1) Retake the course — restart the required modules (and Deep Dives) from the beginning, or (2) Try a new quiz — a full, fresh 8-question summative assessment (new item bank).
- **Framing:** Warm and forward-looking — "Let's go over a few things together, then you can pick whichever path feels right." Not a punishment.
